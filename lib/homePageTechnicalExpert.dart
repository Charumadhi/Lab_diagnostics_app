...
import 'main.dart';
import 'patientTestViewPage.dart';

class Homepagetechnicalexpert extends StatefulWidget {
  final String email;

  Homepagetechnicalexpert({required this.email});

  @override
  _HomePageTechnicalExpertState createState() => _HomePageTechnicalExpertState();
}

class _HomePageTechnicalExpertState extends State<Homepagetechnicalexpert> {
  ...
  Future<void> fetchLabDetails() async {
    try {
      final response = await http.post(
        Uri.parse('http://10.0.2.2:3000/getLabDetails'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        print('Data received from server: $data'); // Debugging: Print the data received

        setState(() {
          labDetails = data['labDetails']; // Access the 'labDetails' key
        });

        print('Lab details set in state: $labDetails'); // Debugging: Print the lab details set in state
      } else {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load lab details')),
      );
    }
  }
          ...
          children: <Widget>[
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => PatientsViewPage(labUniqueId: labDetails!['lab_unique_id'])),
                );
              },
            ),
            ListTile(
...
void main() {
  runApp(MaterialApp(
    home: Homepagetechnicalexpert(email: 'example@example.com'),
  ));
}
